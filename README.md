...
4. **Original question + 20 candidates → Cohere Rerank (top_n=4)** — a cross-encoder re-scores the candidates against the *real* question and keeps the best 4.
5. **Reranked docs → Parser → Answer Prompt → Language Model → Chat Output** — the top 4 chunks are flattened to text and combined with the original question into a grounded prompt for the final answer.
...
**Where the AI calls happen:** entirely inside the Langflow flow (`Advance_RAG_Flow.json`) — there is no separate application backend in this project. The flow is invoked over HTTP:
...
1. **Import the flow**: Langflow dashboard → **My Flows** → **Import** → select `Advance_RAG_Flow.json`.
...
Any document Langflow's **Read File** component supports (PDF, TXT, MD, CSV, DOCX). The included sample, `sample_docs/velocart_platform_docs.pdf`, is a 6-section synthetic product/API documentation file generated by `generate_sample_pdf.py`, deliberately built with:
...
├── Advance_RAG_Flow.json              # exported Langflow flow definition (secrets stripped)
├── generate_sample_pdf.py             # generates sample_docs/velocart_platform_docs.pdf
├── sample_docs/
...
- [ ] Import `Advance_RAG_Flow.json` into Langflow
...
### Testing Findings (from T01–T05 QA runs)

- **T01 (precision)** — passed: reranking correctly isolated the relevant section (Account Lockout Policy) over similar-sounding adjacent sections.
- **T02 (HyDE ambiguity)** — initially failed silently on a deliberately ambiguous query (answered only one of two valid interpretations); resolved by adjusting the prompt/rerank configuration to surface both interpretations explicitly.
- **T04 (structured content)** — the pricing table survived intact with no cross-row contamination, though investigation showed the default semantic-chunking sensitivity (`breakpoint_threshold_type: percentile`) can merge entire sections into oversized chunks, which is worth tuning further (lower the threshold or switch to `standard_deviation`) rather than assumed to be correctly granular by default.
